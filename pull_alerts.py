...
ESCALATION_POLICIES = os.environ.get("ESCALATION_POLICIES")

if not PAGERDUTY_API_TOKEN or not ESCALATION_POLICIES:
    try:
        import settings
...
pagerduty_service = pygerduty.v2.PagerDuty(PAGERDUTY_API_TOKEN)
LOCAL_TZ = tz.tzlocal()
...
class FormattedIncident(object):
    ...
    def _csv_output(self):
        return '{},{},{},{},{},"{}"'.format(
            self.created_on.strftime("%m-%d-%Y"),
    ...
    try:
        recent_incidents = list(
            pagerduty_service.incidents.list(
                service_ids=service_ids, since=datetime.now(tz=LOCAL_TZ) - time_window
            )
        )
...
def print_all_incidents(
    ...
    for escalation_policy in ESCALATION_POLICIES:
        services.extend(
            list(pagerduty_service.escalation_policies.show(escalation_policy).services)
        )

    recent_incidents = recent_incidents_for_services(
        services, timedelta(days=time_window_days)
    )
    formatted_incidents = get_formatted_incidents(recent_incidents)

    (
        all_incidents,
        sorted_description_to_incident_list,
        sorted_service_to_incident_list,
    ) = sort_incidents(formatted_incidents, group_by_description, group_by_service)
...
def get_formatted_incidents(recent_incidents):
    ...
    for incident in recent_incidents:
        ...
        if hasattr(incident, "title"):
            ...
            logger.warning(
                "action=get_description status=not_found incident={}".format(incident)
            )
        formatted_incident.created_on = dateutil.parser.parse(
            incident.created_at
        ).astimezone(LOCAL_TZ)
        ...
        formatted_notes = []
        for note in notes:
            formatted_notes.append("{}: {}".format(note.user.summary, note.content))
...
def sort_incidents(all_incidents, group_by_description, group_by_service):
    ...
    # Sort by desc count
    sorted_description_to_incident_list = OrderedDict(
        sorted(
            description_to_incident_list.items(), key=lambda x: len(x[1]), reverse=True
        )
    )
    sorted_service_to_incident_list = OrderedDict(
        sorted(service_to_incident_list.items(), key=lambda x: len(x[1]), reverse=True)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--include-incidents-as-blockquote",
        action="store_true",
        default=False,
        help="Include raw incident log as markdown blockquote",
    )
    parser.add_argument("--days", type=int, default=7, help="time window days")
    parser.add_argument(
        "--print-csv", action="store_true", default=False, help="Print in CSV format"
    )
